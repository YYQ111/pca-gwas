...
option_list = list(
  ...
  make_option(c("-k", "--k_subpops"), type = "integer", default =510, 
...
simple <- opt$simple
  ...
  rmsd_list<-matrix(0,2,100)
  auc_list<-matrix(0,2,100)
  style <- read_tsv('style.txt', col_types = 'cci')
  for (pcs in 2:80){
    for (j in 1:10){
      sim_geno_trait_k3 <- function(
        ...
        k_subpops = 50, # number of intermediate subpopulations
        ...
        inbr_subpops <- rep(fst,50) # rescaled Fst vector for intermediate subpops
  ...
  M_rmsd[j,pcs]<-p[1]
    ...
    rmsd_list[1,pcs]<-mean(M_rmsd[,pcs])
    rmsd_list[2,pcs]<-var(M_rmsd[,pcs])
    auc_list[1,pcs]<-mean(M_auc[,pcs])
    auc_list[2,pcs]<-var(M_auc[,pcs])
    
  }
  setwd("/dscrhome/yy222/fix_k/k_50")
  
  write.table(M_auc, file="auc_k_fixed_k_50.txt")
  write.table(M_rmsd, file="rmsd_k_fixed_k_50.txt")
