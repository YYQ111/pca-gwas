...
simple <- opt$simple
  ...
  for (k in 2:80){
    for (j in 1:10){
      sim_geno_trait_k3 <- function(
        ...
        k_subpops = k, # number of intermediate subpopulations
  ...
  M_rmsd[j,k]<-p[1]
    ...
    rmsd_list[1,k]<-mean(M_rmsd[,k])
    rmsd_list[2,k]<-var(M_rmsd[,k])
    auc_list[1,k]<-mean(M_auc[,k])
    auc_list[2,k]<-var(M_auc[,k])
    
  }
  setwd("/dscrhome/yy222/fix_pcs/pcs_50")
  
  write.table(M_auc, file="auc_k_fixed_pcs_50.txt")
  write.table(M_rmsd, file="rmsd_k_fixed_pcs_50.txt")
